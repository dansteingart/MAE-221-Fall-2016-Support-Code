...
void setup() //run this loop just once upon start, or upon reset
{
  ...
  Particle.variable("big_brother",publishString);
...
void loop() //repeat this loop forever
{
    ...
    for (int j = 0; j < samps; j++) //sum values samps times
   {
    ...
    Particle.publish("big_brother",publishString);

    digitalWrite(7,0);

    delay(waiter);  //Wait waiter ms before sending taking and sending next measurement.
